...
// Maze struxt is to setup the variable to make the maze
// c (cell) -> cell
// h (hor) -> horizontal
// v (ver) -> vertical
// c,h,v are single array
// cell, hor, ver are double array
type Maze struct {
	c, h, v        []byte
	cell, hor, ver [][]byte
}

// DrawMaze = Drawing the maze function
...
func (m *Maze) generator() {
	//Allocate the maze using recursive method
...
func (m *Maze) recursion(row, col int) {
	...
	for _, wall := range rand.Perm(4) {
		switch wall {
		// check if cells up is out or not
		case up:
			if row > 0 && m.cell[row-1][col] == 0 {
				m.hor[row][col] = 0
				m.recursion(row-1, col)
			}
		// check of cells down is out or not
		case down:
			if row < len(m.cell)-1 && m.cell[row+1][col] == 0 {
				m.hor[row+1][col] = 0
				m.recursion(row+1, col)
			}
		// check if cells left is out or not
		case left:
			if col > 0 && m.cell[row][col-1] == 0 {
				m.ver[row][col] = 0
				m.recursion(row, col-1)
			}
		// check if cells right is out or not
		case right:
			if col < len(m.cell)-1 && m.cell[row][col+1] == 0 {
				m.ver[row][col+1] = 0
				m.recursion(row, col+1)
			}
		}
